...
public class ArtifactDownloadManagerTest {
  ...
  public void downloadArtifacts() throws JsonProcessingException {
    ...
    kafkaTemplate.send(topic, objectMapper.writeValueAsString(notificationData));

    stubFor(get(urlEqualTo("/sdc/v1/catalog/services/DemovlbCds/1.0/artifacts/service-DemovlbCds-csar.csar"))
        ...
        .willReturn(aResponse()
          ...
          .withBodyFile("noVnfcFilesArchive.csar")));

    ObjectMapper objectMapper = new ObjectMapper();
    List<BabelArtifact> babelArtifact = List.of(
    ...
    stubFor(
        post(urlEqualTo("/services/babel-service/v1/app/generateArtifacts"))
            ...
            .withRequestBody(matchingJsonPath("$.artifactName", equalTo("service-DemovlbCds-csar.csar")))
            // .withRequestBody(matchingJsonPath("$.artifactVersion", equalTo("1.0")))
            .withRequestBody(matchingJsonPath("$.csar", matching(".*")))
            .willReturn(
                aResponse()
                    .withHeader("Content-Type", "application/json")
                    .withBody(objectMapper.writeValueAsString(babelArtifact))));
    
        // .willReturn(aResponse().withHeader("Content-Type", "application/json").withBodyFile("services.json");
    ...
    artifactInfo.setArtifactName("service-DemovlbCds-csar.csar");
    artifactInfo.setArtifactVersion("1.0");
    artifactInfo.setArtifactURL("/sdc/v1/catalog/services/DemovlbCds/1.0/artifacts/service-DemovlbCds-csar.csar");
